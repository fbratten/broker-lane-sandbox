...
supplied a non-string `request_id` (only string ids are echoed; the request is
rejected with `request_error`), or when the request file could not be read or parsed
as JSON at all (as in the example above).
